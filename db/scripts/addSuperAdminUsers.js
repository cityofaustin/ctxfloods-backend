...
const addSuperAdminUsers = (client) => {
  ...
  return client.query(format(
    `
      ...
      select floods.register_user(text 'Floods', text 'Graphql', text 'API', integer '1337', text '000-0000', text 'graphql@flo.ods', text %L, text 'floods_super_admin');
    `,
    process.env.PG_SUPER_ADMIN_PW, process.env.PG_API_PW
  ))
}
